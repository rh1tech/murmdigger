...
# Build configurations: "BOARD CPU_SPEED DESCRIPTION"
CONFIGS=(
    "M1 252 Murmulator"
    "M2 252 FRANK"
)
...
for config in "${CONFIGS[@]}"; do
    ...
    echo -e "  Board: $BOARD ($DESC) | CPU: ${CPU} MHz | USB HID: enabled"
    ...
    # Configure with CMake (USB HID enabled for keyboard support)
    cmake .. -DPICO_BOARD=pico2 -DBOARD_VARIANT="$BOARD" -DCPU_SPEED="$CPU" -DUSB_HID_ENABLED=1 > /dev/null 2>&1
